...
@php
    ...
    foreach ($overflowItems as $oi) {
        if (request()->routeIs($oi['active'])) {
...
<div
    x-data="{ moreOpen: false, mobileProfileOpen: false }"
    class="min-h-screen bg-[#F7F9FC] dark:bg-[#090D16] text-slate-900 dark:text-slate-100 flex flex-col transition-colors duration-300 relative overflow-x-hidden"
>
    ...
    <header class="fixed top-0 left-0 right-0 h-16 bg-white/80 dark:bg-[#0b0f19]/80 backdrop-blur-md border-b border-slate-200/50 dark:border-slate-800/50 z-50 flex items-center justify-between px-5 transition-colors duration-300">
        <a href="{{ route('admin.dashboard') }}" class="flex items-center gap-2.5">
            ...
            <div>
                ...
                <span class="block text-[9px] font-semibold text-slate-400 -mt-0.5">Admin Panel</span>
            </div>
        </a>
            ...
            <div class="relative" x-on:click.outside="mobileProfileOpen = false">
                ...
                >
                    <div class="px-3 py-2.5">
                        ...
                        <p class="text-[10px] text-slate-400 truncate mt-1 leading-none">Admin</p>
                    </div>
                    <div class="h-px bg-slate-100 dark:bg-slate-800 my-1"></div>
                    <a href="{{ route('admin.settings') }}" class="flex items-center gap-2 px-3 py-2 rounded-lg text-[12px] text-slate-600 dark:text-slate-300 hover:bg-slate-50 dark:hover:bg-slate-800 transition">
    ...
    {{-- ═══════════ "Lainnya" Overflow Menu (slides up above bottom bar) ═══════════ --}}
    <div
        x-show="moreOpen"
        x-cloak
        ...
        class="fixed bottom-24 left-4 right-4 bg-white dark:bg-[#0F1524] border border-slate-200/50 dark:border-slate-800/50 rounded-2xl shadow-[0_12px_40px_rgba(0,0,0,0.1)] dark:shadow-[0_12px_40px_rgba(0,0,0,0.4)] z-50 p-2 max-w-lg mx-auto backdrop-blur-md"
    >
        @foreach ($overflowItems as $oi)
            @php $oiActive = request()->routeIs($oi['active']); @endphp
            <a
                href="{{ route($oi['route']) }}"
        ...
        <form method="POST" action="{{ route('logout') }}">
            ...
            <button
                type="submit"
                class="flex w-full items-center gap-3 rounded-xl px-4 py-3 text-sm font-bold text-red-600 transition hover:bg-red-50 dark:text-red-300 dark:hover:bg-red-500/10"
            >
                <svg class="h-5 w-5" viewBox="0 0 24 24" fill="none" stroke="currentColor">
    ...
    <nav class="fixed bottom-4 left-4 right-4 h-16 bg-white/90 dark:bg-[#0b0f19]/90 border border-slate-200/40 dark:border-slate-800/40 shadow-[0_8px_30px_rgba(0,0,0,0.06)] dark:shadow-[0_8px_30px_rgba(0,0,0,0.35)] rounded-2xl z-50 flex items-center justify-around px-2 backdrop-blur-md max-w-lg mx-auto transition-colors duration-300">
            ...
            >
                <span class="mb-1 transition-transform duration-300 group-hover:scale-110 {{ $isActive ? 'scale-110' : '' }}">
                    @switch($item['icon'])
                        ...
                        @case('students')
                            <svg class="h-[18px] w-[18px]" viewBox="0 0 24 24" fill="none" stroke="currentColor">
                                <path stroke-linecap="round" stroke-linejoin="round" stroke-width="1.9" d="M17 20v-1.5A4.5 4.5 0 0 0 12.5 14h-5A4.5 4.5 0 0 0 3 18.5V20" />
                                <path stroke-linecap="round" stroke-linejoin="round" stroke-width="1.9" d="M10 10a4 4 0 1 0 0-8 4 4 0 0 0 0 8ZM21 20v-1.5a3.5 3.5 0 0 0-2.5-3.37M16 3.13a3.5 3.5 0 0 1 0 6.74" />
                            </svg>
                            @break
                        @case('teachers')
                            <svg class="h-[18px] w-[18px]" viewBox="0 0 24 24" fill="none" stroke="currentColor">
        ...
        >
            <span class="mb-1 transition-transform duration-300 group-hover:scale-110 {{ $isOverflowActive ? 'scale-110' : '' }}">
                <svg class="h-[18px] w-[18px]" viewBox="0 0 24 24" fill="none" stroke="currentColor">
            ...
            <span class="text-[9px] font-semibold tracking-tight leading-none">
                Lainnya
            </span>
        </button>
    </nav>

    {{-- Backdrop for closing "Lainnya" --}}
    <div
        x-show="moreOpen"
        x-cloak
