...
<head>
    ...
    <title>Input Absensi - PerCikAIS Admin</title>
...
<body class="font-sans antialiased bg-gray-50 dark:bg-[#050B14] text-gray-900 dark:text-white flex h-screen overflow-hidden transition-colors duration-300">
    ...
    <aside class="w-64 bg-white dark:bg-[#0A0F1C] border-r border-gray-200 dark:border-gray-800 flex flex-col justify-between overflow-y-auto transition-colors">
        <div class="p-6">
            ...
            <nav class="space-y-2 text-sm font-medium">
                ...
                <a href="{{ route('admin.attendance') }}" class="flex items-center gap-3 py-3 px-4 rounded-xl bg-blue-50 dark:bg-blue-500/10 text-blue-600 dark:text-blue-500 border border-blue-200 dark:border-blue-500/20 transition-colors">
    ...
    <main class="flex-1 p-8 overflow-y-auto transition-colors">
        <div class="mb-8">
            <h1 class="text-2xl font-bold text-gray-900 dark:text-white">Input Absensi Harian</h1>
            <p class="text-sm text-gray-500 dark:text-gray-400">Catat kehadiran siswa untuk pertemuan kelas hari ini.</p>
        </div>
        ...
        <div class="grid grid-cols-1 lg:grid-cols-3 gap-8">
            ...
            <div class="lg:col-span-1 bg-white dark:bg-[#0A0F1C] p-6 rounded-3xl border border-gray-200 dark:border-gray-800 shadow-sm h-fit transition-colors">
                <h2 class="font-bold text-gray-900 dark:text-white mb-6 border-b dark:border-gray-800 pb-3">Form Kehadiran</h2>
                ...
                <form action="{{ route('admin.attendance.store') }}" method="POST" class="space-y-5 transition-colors">
                    ...
                    <div>
                        <label class="block text-sm font-medium text-gray-700 dark:text-gray-300 mb-1">Mata Kuliah / Kelas</label>
                        <input type="text" name="subject_name" required placeholder="Cth: Keamanan Sistem" class="w-full rounded-xl border-gray-300 dark:border-gray-700 dark:bg-[#050B14] dark:text-white focus:border-blue-500 focus:ring-blue-500 text-sm shadow-sm transition-colors">
                    </div>

                    <div>
                        <label class="block text-sm font-medium text-gray-700 dark:text-gray-300 mb-1">Tanggal Pertemuan</label>
            ...
            <div class="lg:col-span-2 bg-white dark:bg-[#0A0F1C] p-6 rounded-3xl border border-gray-200 dark:border-gray-800 shadow-sm transition-colors">
                <h2 class="font-bold text-gray-900 dark:text-white mb-6 border-b dark:border-gray-800 pb-3">Input Hari Ini</h2>
                    ...
                    <table class="w-full text-left">
                        <thead>
                            <tr class="text-xs uppercase text-gray-400 dark:text-gray-500 border-b dark:border-gray-800 tracking-wider">
                                ...
                                <th class="pb-3 font-semibold">Mata Kuliah</th>
                        ...
                        <tbody class="text-sm divide-y divide-gray-100 dark:divide-gray-800 transition-colors overflow-hidden">
                            ...
                            <tr class="hover:bg-gray-50 dark:hover:bg-[#121A2F]/50 transition-colors rounded-xl overflow-hidden">
                                ...
                                <td class="py-4 text-gray-500 dark:text-gray-400">{{ $absen->subject_name }}</td>
                                <td class="py-4 text-center">
                                    @if($absen->status == 'Hadir')
                                        <span class="text-xs font-bold px-2.5 py-1.5 bg-green-100 text-green-700 dark:bg-green-500/10 dark:text-green-400 rounded-md uppercase tracking-wide transition-colors">Hadir</span>
                                    @elseif($absen->status == 'Sakit')
                                        <span class="text-xs font-bold px-2.5 py-1.5 bg-yellow-100 text-yellow-700 dark:bg-yellow-500/10 dark:text-yellow-400 rounded-md uppercase tracking-wide transition-colors">Sakit</span>
                                    @elseif($absen->status == 'Izin')
                                        <span class="text-xs font-bold px-2.5 py-1.5 bg-blue-100 text-blue-700 dark:bg-blue-500/10 dark:text-blue-400 rounded-md uppercase tracking-wide transition-colors">Izin</span>
                                    @else
                                        <span class="text-xs font-bold px-2.5 py-1.5 bg-red-100 text-red-700 dark:bg-red-500/10 dark:text-red-400 rounded-md uppercase tracking-wide transition-colors">Alpa</span>
                                    @endif
                                </td>
                            </tr>
                            @empty
                            <tr>
                                <td colspan="3" class="py-6 text-center text-gray-500 dark:text-gray-400">Belum ada absen diinput hari ini.</td>
                            </tr>
                            @endforelse
                        </tbody>
                    </table>
